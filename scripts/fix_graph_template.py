#!/usr/bin/env python3
"""Rewrite graph template post with valid Java code blocks."""

from pathlib import Path

ROOT = Path(__file__).resolve().parent.parent
TARGET = ROOT / "_posts/2025-10-29-leetcode-templates-graph.md"

BLOCKS = {
    "bfs_grid": """static int bfsGrid(char[][] g, int si, int sj, int ti, int tj) {
    ...
    "multi_bfs": """static int multiBfs(char[][] g, List<int[]> sources) {
    int m = g.length, n = g[0].length;
    int[][] dist = new int[m][n];
    for (int[] row : dist) Arrays.fill(row, -1);
    ArrayDeque<int[]> q = new ArrayDeque<>();
    ...
    "bfs_mask": """static int bfsMask(int n, List<List<Integer>> g, int start) {
    int full = (1 << n) - 1;
    ...
    "topo_kahn": """static List<Integer> topoKahn(int n, List<List<Integer>> g) {
    int[] indeg = new int[n];
    ...
    "topo_dfs": """static List<Integer> topoDfs(int n, List<List<Integer>> g) {
    int[] color = new int[n];
    List<Integer> order = new ArrayList<>();
    boolean[] ok = {true};
    for (int i = 0; i < n; i++) {
...
private static void dfs(int u, List<List<Integer>> g, int[] color,
    ...
    "dijkstra": """static long[] dijkstra(int n, List<List<int[]>> g, int s) {
    long INF = 1L << 60;
    ...
    "dijkstra_disappear": """static int[] dijkstraDisappear(int n, List<List<int[]>> g, int[] disappear) {
    int[] dist = new int[n];
    Arrays.fill(dist, -1);
    dist[0] = 0;
    ...
    "dijkstra_grid_open": """static long dijkstraGridOpen(int[][] open) {
    ...
    "bfs01": """static int[] bfs01(int n, List<List<int[]>> g, int s) {
    int[] dist = new int[n];
    ...
    "bellman_ford": """static long[] bellmanFordK(int n, int[][] edges, int src, int k) {
    long INF = 1L << 60;
    long[] dist = new long[n];
    Arrays.fill(dist, INF);
    dist[src] = 0;
    ...
    "bridges": """static List<int[]> bridges(int n, List<List<Integer>> g) {
    int timer = 0;
...
def main() -> None:
    ...
    start = text.index("## BFS (unweighted)")
    end = text.index("## More templates")
    body = f"""## BFS (unweighted)

Grid: 4-directional. Use for shortest path when all edges have weight 1.

{fence(BLOCKS['bfs_grid'])}
