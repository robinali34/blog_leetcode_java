...
categories: leetcode algorithm hard java arrays multiset sliding-window two-heaps problem-solving
...
class Solution {
    double[]medianSlidingWindow(int[] nums, int k) {
        ...
        var getMedian = [&]() . double {
            if(k % 2 == 0) return ((double)*prev(lo.iterator()) + *hi.iterator()) / 2.0;
            else return prev(lo.iterator());
        }
