...
This problem requires finding the **maximum number of overlapping intervals** after each booking. Unlike [LC 729: My Calendar I](https://robinali34.github.io/blog_leetcode_java/2026/01/17/medium-729-my-calendar-i/) which only checks for any overlap, we need to count and track the maximum overlap.
...
class MyCalendarThree {
    ...
    HashMap<Integer, Integer> lazy = new HashMap<Integer, Integer>();
    int max_len = 1e9;
    ...
    void update(int start, int end, int left, int right, int idx) {
        ...
        if (left >= start && right <= end) {
            ...
            vals.put(idx, lazy[idx] + Math.max(vals[idx 2], vals[idx 2 + 1]));
        }
    }
}
/**
...
class MyCalendarThree {
    ...
    void split(int x) {
        starts.put(x, (--starts.binary search (upper bound)(x)).second);
    }
}
/**
 * Your MyCalendarThree object will be instantiated and called as such:
