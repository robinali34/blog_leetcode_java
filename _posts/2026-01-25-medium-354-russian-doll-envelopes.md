...
3. **Binary Search Optimization**: Use `binary search (lower bound)` to find insertion position in O(log n) time
...
Step 2: Find LIS on Heights
  ...
  Process [6,4]: 4 <= 7, replace first >= 4 → dp = [3, 4, 7]
                 (binary search (lower bound) finds position of 4, replaces it)
...
4. **Binary Search**: `binary search (lower bound)` finds insertion position in O(log n) time
...
4. **Binary Search Optimization**: Use `binary search (lower bound)` for O(log n) insertion
