...
class Solution {
        ...
        stack<TreeNode[]> st;
        st.offer(new int[] {root, targetSum});

        while (!st.isEmpty()) {
            int[] nodepair = st.peek(); int node = nodepair[0]; int remaining = nodepair[1];
            st.poll();
            ...
            if (node.right) {
                st.offer({node.right, remaining - node.val});
            }
            if (node.left) {
                st.offer({node.left, remaining - node.val});
            }
        }
...
class Solution {
        ...
        queue<TreeNode[]> q;
        q.offer(new int[] {root, targetSum});

        while (!q.isEmpty()) {
            auto [node, remaining] = q.get(0);
            q.poll();
            ...
            if (node.left) {
                q.offer({node.left, remaining - node.val});
            }
            if (node.right) {
                q.offer({node.right, remaining - node.val});
            }
        }
