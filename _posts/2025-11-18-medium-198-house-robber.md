...
static int rob(int[] nums) {
    ...
    return dp[nums.length];  // Maximum for all houses
}
```
...
dp[i+1] = Math.max(...);  // Current house at index i
```

**0-Indexed (Standard):**
```java
dp[0] = nums[0];     // First house
dp[1] = Math.max(nums[0], nums[1]);  // First two houses
dp[i] = Math.max(...);    // Current house at index i
```
