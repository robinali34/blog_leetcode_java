...
**Why use a custom comparator struct?** Defining `Compare` as a struct makes the comparator reusable and avoids lambda overhead. The `priority_queue` in Java is a max-heap by default, so we reverse the comparison (`a->val > b->val`) to get min-heap behavior.
