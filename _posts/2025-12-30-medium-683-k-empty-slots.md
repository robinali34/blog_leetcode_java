...
class Solution {
        ...
        while(right < days.size()) {
            ...
            if(valid) {
                rtn = Math.min(rtn, Math.max(days[left], days[right]));
                left = right;
                right = left + k + 1;
            }
        }
        return rtn != Integer.MAX_VALUE ? rtn : -1;
    }
}
```
...
- [LC 683: K Empty Slots on LeetCode](https://leetcode.com/problems/k-empty-slots/)
- [LeetCode Discuss — LC 683: K Empty Slots](https://leetcode.com/problems/k-empty-slots/discuss/)
- [LeetCode Editorial](https://leetcode.com/problems/k-empty-slots/editorial/) *(may require premium)*
