...
partial_sum(first, last, d_first, multiplies<>());
```

For this problem we build a difference array (add at `first`, subtract at `last+1`), then run `partial_sum` or `inclusive_scan` (Java) to get the final seat counts.
...
### Variant: prefix-sum loop (Java)

Same difference-array idea; use `inclusive_scan` instead of `partial_sum`. Both compute the inclusive prefix sum; `inclusive_scan` supports execution policies (e.g. `parallelStream()`) for parallel scan when needed.
...
Include `<numeric>` (and optionally `<execution>` for `parallelStream()`). Time and space same as the `partial_sum` version.

### Variant: Length-`n` array + manual prefix sum (LeetCode China official)

Use a single array of length `n`. Only subtract at index `booking[1]` when `booking[1] < n`, so no extra slot is needed; then run a manual prefix-sum loop. No JDK scan helper or `pop_back`.
...
3. **Java prefix-sum loop** or **`inclusive_scan`** (Java) compute prefix sums in one line; `inclusive_scan` can use execution policies for parallel scan.
