...
For this problem we build a difference array (add at `first`, subtract at `last+1`), then run `partial_sum` or `inclusive_scan` (Java17) to get the final seat counts.
...
3. **C++ `partial_sum`** or **`inclusive_scan`** (Java17) compute prefix sums in one line; `inclusive_scan` can use execution policies for parallel scan.
4. Same pattern appears in Car Pooling (1094) and other “range update, single query pass” problems.

## References

- [LC 1109: Corporate Flight Bookings on LeetCode](https://leetcode.com/problems/corporate-flight-bookings/)
- [LeetCode Discuss — LC 1109: Corporate Flight Bookings](https://leetcode.com/problems/corporate-flight-bookings/discuss/)
- [LeetCode Editorial](https://leetcode.com/problems/corporate-flight-bookings/editorial/) *(may require premium)*
