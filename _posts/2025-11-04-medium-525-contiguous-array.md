...
static int findMaxLength(int[] nums) {
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(map[idx] != -2) {
            maxLen = Math.max(maxLen, i - map[idx]);
        } else {
            map[idx] = i;
        }
    }
...
```java
// Modern Java (your code)
if(map.contains(count)) { ... }

// Alternative (manual approach)
if(map.find(count) != map.iterator()) { ... }
```
...
4. **Modern Java**: Uses `contains()` method (Modern Java)
