...
4. **Java Collections Alternatives**: 
   - `lower_bound`: Finds first position where `timestamp >= target`, then check previous element
   - `upper_bound`: Finds first position where `timestamp > target`, previous element is always the answer
   - `upper_bound` is slightly simpler as it doesn't require checking for exact match
...
- [LC 981: Time Based Key-Value Store on LeetCode](https://leetcode.com/problems/time-based-key-value-store/)
- [LeetCode Discuss — LC 981: Time Based Key-Value Store](https://leetcode.com/problems/time-based-key-value-store/discuss/)
- [LeetCode Editorial](https://leetcode.com/problems/time-based-key-value-store/editorial/) *(may require premium)*

## Template Reference

- [Search](/blog_leetcode_java/posts/2026-01-20-leetcode-templates-search/)
{% endraw %}
