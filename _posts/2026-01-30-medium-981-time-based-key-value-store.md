...
2. **Get Operation**:
   - Use `binary search (lower bound)` with custom comparator to find first position where `timestamp >= target_timestamp`
   - If exact match found, return that value
   - If `binary search (lower bound)` returns `begin()`, no valid timestamp exists, return `""`
...
2. **Get Operation**:
   - Use `binary search (upper bound)` with custom comparator to find first position where `timestamp > target_timestamp`
   - The comparator `[](int ts, const pair<int, string>& p) { return ts < p.first; }` compares target timestamp with element's timestamp
   - If `binary search (upper bound)` returns `begin()`, no valid timestamp exists (all timestamps > target), return `""`
   - Otherwise, return the value at `prev(it)` (the largest timestamp <= target_timestamp)

### Why This Works:

- **Upper Bound**: Finds the first position where `timestamp > target_timestamp`
- **Previous Element**: The element before `binary search (upper bound)` is the largest timestamp <= target_timestamp
...
4. **JDK Alternatives**: 
   - `binary search (lower bound)`: Finds first position where `timestamp >= target`, then check previous element
   - `binary search (upper bound)`: Finds first position where `timestamp > target`, previous element is always the answer
   - `binary search (upper bound)` is slightly simpler as it doesn't require checking for exact match
