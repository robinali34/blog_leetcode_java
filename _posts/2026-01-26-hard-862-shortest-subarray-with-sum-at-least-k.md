...
This problem is similar to [LC 209: Minimum Size Subarray Sum](https://robinali34.github.io/blog_leetcode_java/2026/01/26/medium-209-minimum-size-subarray-sum/), but with a critical difference: **the array can contain negative numbers**.
...
```java
// import java.util.*;
ArrayDeque<Integer> q = new ArrayDeque<>();  // Stores indices of prefix sums
for(int i = 0; i <= N; i++) {
    long curSum = preSum[i];
